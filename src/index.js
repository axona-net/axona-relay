...
//   (metrics: demand-driven in the kernel since v4.12.0 — any root publishes
//    snapshots while a metric topic is subscribed; no relay loop, no env knob)
//
...
async function main() {
  ...
  // Metrics are DEMAND-DRIVEN in the kernel since v4.12.0 (METRICSON): any
  // node that is root for a topic publishes snapshots to metricTopic(T) while
  // a subscriber's lease is fresh — relays included, with no special loop.
  // The old push-based relay metrics loop is retired; it would double-publish.
  present.logLine(`{cyan-fg}metrics{/} demand-driven (kernel METRICSON) — snapshots publish while a metric topic has subscribers`);
}
