...
# (3 relays each for grizzly / eagle / bison) against wss://testnet.axona.net.
...
#   REGIONS="grizzly eagle bison" PER=3 BRIDGE=wss://testnet.axona.net bash start-backbone.sh
#
# Stop: pkill -f "node src/index.js"
set -u
cd "$(dirname "$0")"
REGIONS="${REGIONS:-grizzly eagle bison}"
PER="${PER:-3}"
...
echo "✓ backbone up ($(echo $REGIONS | wc -w | tr -d ' ') regions × $PER). tail -f relay-logs/tn-eagle-1.log"
